...
function renderQuestion() {
  ...
  // Progress dots & ARIA updates
  const progressPercent = Math.round((currentStep / quizKeys.length) * 100);
  ...
  const optionsHTML = data.options.map((opt, i) => {
    ...
    const ariaPressed = previousAnswer === opt.tons ? 'true' : 'false';
    return `
      <button class="quiz-option${isSelected}" data-key="${key}" data-index="${i}" id="opt-${i}" aria-pressed="${ariaPressed}">
...
function renderInsightCallout(breakdown, total) {
  ...
  el.innerHTML = `<span role="img" aria-label="Tip">💡</span> <strong>Key insight:</strong> ${insights[top.key]}`;
}
  ...
  container.innerHTML = breakdown.map((item, i) => {
    ...
    return `
      <div class="breakdown-item">
        <span class="breakdown-icon" role="img" aria-hidden="true">${item.icon}</span>
        <div class="breakdown-info">
          ...
          <div class="breakdown-bar-bg" role="progressbar" aria-valuenow="${item.percent}" aria-valuemin="0" aria-valuemax="100" aria-label="${item.label} contribution percentage">
  ...
  container.innerHTML = keys.map(key => {
    ...
    return `
      <div class="equiv-card">
        <div class="equiv-icon" role="img" aria-hidden="true">${eq.icon}</div>
...
function renderActions(topCategory) {
  ...
  container.innerHTML = actions.map((action, i) => `
    <div class="action-card">
      <div class="action-icon" role="img" aria-hidden="true">${action.icon}</div>
...
function init() {
  ...
  if (optContainer) {
    optContainer.addEventListener('click', (e) => {
      ...
      if (optionBtn) {
        const optionIndex = parseInt(optionBtn.dataset.index, 10);
        const optionKey = optionBtn.dataset.key;
        selectOption(optionKey, optionIndex);
      }
    });
  }
}

// Run immediately since script is at bottom of HTML body
init();
